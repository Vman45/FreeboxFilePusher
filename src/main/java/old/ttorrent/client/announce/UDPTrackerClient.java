 ...
 * exchange where the peer is first required to establish a "connection" with
 * the tracker by sending a connection request message and retreiving a
 * connection ID from the tracker to use in the following announce request
 * messages (valid for 2 minutes).
 * </p>
 *
 * <p>
 * It also contains a backing-off retry mechanism (on a 15*2^n seconds scheme),
 * in which if the announce request times-out for more than the connection ID
 * validity period, another connection request/response exchange must be made
 * before attempting to retransmit the announce request.
 * </p>
 *
 * @author mpetazzoni
 */
public class UDPTrackerClient extends TrackerClient {
	...
	protected static final Logger logger = LoggerFactory
			.getLogger(UDPTrackerClient.class);
	...
	protected UDPTrackerClient(SharedTorrent torrent, Peer peer, URI tracker)
		...
		this.address = new InetSocketAddress(tracker.getHost(),
				tracker.getPort());
	...
	public void announce(AnnounceRequestMessage.RequestEvent event,
		...
		logger.info(
				...
				new Object[] { this.formatAnnounceEvent(event),
						this.torrent.getUploaded(),
						this.torrent.getDownloaded(), this.torrent.getLeft() });

		State state = State.CONNECT_REQUEST;
		int maxAttempts = AnnounceRequestMessage.RequestEvent.STOPPED
				.equals(event) ? UDP_MAX_TRIES_ON_STOPPED : UDP_MAX_TRIES;
		int attempts = -1;
		...
		try {
			...
			while (++attempts <= maxAttempts) {
				...
				case CONNECT_REQUEST:
					...
					try {
						this.handleTrackerConnectResponse(UDPTrackerMessage.UDPTrackerResponseMessage
	 ...
	 *
	 * <p>
	 * Verifies the transaction ID of the message before passing it over to any
	 * registered {@link AnnounceResponseListener}.
	 * </p>
	 *
	 * @param message
	...
	private UDPAnnounceRequestMessage buildAnnounceRequest(
		...
		return UDPAnnounceRequestMessage.craft(this.connectionId,
				transactionId, this.torrent.getInfoHash(), this.peer
						.getPeerId().array(), this.torrent.getDownloaded(),
				this.torrent.getUploaded(), this.torrent.getLeft(), event,
				this.peer.getAddress(), 0,
