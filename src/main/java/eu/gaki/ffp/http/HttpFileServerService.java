...
public class HttpFileServerService implements Container {
     ...
     */
    private void sendFile(final Request request, final Response response, final Path pathToServe,
	...
	if (typeValue.length == 2 && "bytes".equals(typeValue[0].trim())) {
		...
		try (WritableByteChannel output = response.getByteChannel();
		    ...
		    response.setValue("Content-Range",
			    "bytes " + firstBytePosition + "-" + lastBytePosition + "/" + lengthBytes);
