...
public class HttpFileServer {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(HttpFileServer.class);
	 ...
	 */
	public HttpFileServer(final Properties configuration,
	...
	public void start() {
		if (connection == null) {
			httpFileServerExecutor
					.execute(() -> {
						LOGGER.info("Starting HTTP file server on {}...",
								address);
						try {
							final SocketProcessor processor = new ContainerSocketProcessor(
									httpFileServerService);
							connection = new SocketConnection(processor);
							connection.connect(address);
						} catch (final IOException ioe) {
							LOGGER.error(
									"Could not start the HTTP file server: {}!",
									ioe.getMessage(), ioe);
	...
	public void stop() {
		if (connection != null) {
			try {
				...
				LOGGER.error("Could not stop the HTTP file server: {}!",
						ioe.getMessage(), ioe);
			}
		}
	}
