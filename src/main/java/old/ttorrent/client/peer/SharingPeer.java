 ...
 *
 * <ul>
 * <li><code>choking</code>, which means we are choking this peer and we're not
 * willing to send him anything for now;</li>
 * <li><code>interesting</code>, which means we are interested in a piece this
 * peer has;</li>
 * <li><code>choked</code>, if this peer is choking and won't send us anything
 * right now;</li>
...
public class SharingPeer extends Peer implements MessageListener {
	 ...
	 *
	 * @param listener
	 *            The activity listener that wants to receive events from this
	 *            peer's activity.
	 */
	...
	public synchronized boolean isSeed() {
		return this.torrent.getPieceCount() > 0
				&& this.getAvailablePieces().cardinality() == this.torrent
						.getPieceCount();
	}
	 ...
	 *
	 * @param channel
	 *            The connected socket channel for this peer.
	 */
	 ...
	 *
	 * @param force
	 *            Force unbind without sending cancel requests.
	 */
	public void unbind(boolean force) {
		if (!force) {
			// Cancel all outgoing requests, and send a NOT_INTERESTED message
			// to
			// the peer.
	 ...
	 *
	 * @param message
	 *            The message to send to the remote peer through our peer
	 *            exchange.
	 */
	public void send(PeerMessage message) throws IllegalStateException {
		if (this.isConnected()) {
			...
			logger.warn(
					"Attempting to send a message to non-connected peer {}!",
					this);
		}
	}
	 ...
	 *
	 * @param piece
	 *            The piece chosen to be downloaded from this peer.
	 */
	...
	private void requestNextBlocks() {
		synchronized (this.requestsLock) {
			if (this.requests == null || this.requestedPiece == null) {
				// If we've been taken out of a piece download context it means
				// our
			...
			while (this.requests.remainingCapacity() > 0
				...
				PeerMessage.RequestMessage request = PeerMessage.RequestMessage
						.craft(this.requestedPiece.getIndex(),
								...
								Math.min(
										(int) (this.requestedPiece.size() - this.lastRequestedOffset),
	...
	/**
	 * Remove the REQUEST message from the request pipeline matching this PIECE
	 * message.
	 *
	 * <p>
	...
	public Set<PeerMessage.RequestMessage> cancelPendingRequests() {
		synchronized (this.requestsLock) {
			Set<PeerMessage.RequestMessage> requests = new HashSet<PeerMessage.RequestMessage>();

			if (this.requests != null) {
				for (PeerMessage.RequestMessage request : this.requests) {
					this.send(PeerMessage.CancelMessage.craft(
							request.getPiece(), request.getOffset(),
							request.getLength()));
	...
	public synchronized void handleMessage(PeerMessage msg) {
		...
		case HAVE:
			...
			synchronized (this.availablePieces) {
				...
				logger.trace("Peer {} now has {} [{}/{}].", new Object[] {
						this, havePiece, this.availablePieces.cardinality(),
			...
			try {
				synchronized (p) {
					if (p.isValid()) {
						...
						this.cancelPendingRequests();
						this.firePeerReady();
						logger.debug("Discarding block for already completed "
								+ p);
						break;
					}
					...
					if (piece.getOffset() + piece.getBlock().capacity() == p
							.size()) {
						p.validate();
	 ...
	 *
	 * <p>
	 * The event contains the peer (this), and the piece number that was sent to
	 * the peer.
	 * </p>
	 *
	 * @param piece
	 ...
	 *
	 * <p>
	 * The event contains the peer that triggered the problem, and the exception
	 * object.
	 * </p>
	 */
	...
	public String toString() {
		return new StringBuilder(super.toString()).append(" [")
				...
				.append((this.interested ? "I" : "i")).append("|")
				.append((this.choking ? "C" : "c"))
				.append((this.interesting ? "I" : "i")).append("|")
				.append(this.availablePieces.cardinality()).append("]")
				.toString();
	}
}
