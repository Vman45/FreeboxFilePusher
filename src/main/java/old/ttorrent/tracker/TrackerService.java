...
public class TrackerService implements Container {
	...
	private void process(final Request request, final Response response,
		...
		final TrackedTorrent torrent = this.torrents.get(announceRequest
				.getHexInfoHash());
		if (torrent == null) {
		...
		if ((event == null || AnnounceRequestMessage.RequestEvent.NONE
				.equals(event)) && currentPeer == null) {
		...
		try {
			peer = torrent.update(event,
					...
					announceRequest.getHexPeerId(), announceRequest.getIp(),
					announceRequest.getPort(), announceRequest.getUploaded(),
					announceRequest.getDownloaded(), announceRequest.getLeft());
		...
		try (final WritableByteChannel channel = Channels.newChannel(body)) {
			announceResponse = HTTPAnnounceResponseMessage.craft(
					...
					TrackedTorrent.MIN_ANNOUNCE_INTERVAL_SECONDS, this.version,
	...
	private HTTPAnnounceRequestMessage parseQuery(final Request request)
		...
		final Map<String, BEValue> params = new HashMap<String, BEValue>();

		try {
			final String uri = request.getAddress().toString();
			for (final String pair : uri.split("[?]")[1].split("&")) {
		...
		if (params.get("ip") == null) {
			params.put("ip", new BEValue(request.getClientAddress()
					.getAddress().getHostAddress(), Torrent.BYTE_ENCODING));
		}
	...
	private void recordParam(final Map<String, BEValue> params,
		...
		try {
			...
			for (final String f : NUMERIC_REQUEST_FIELDS) {
				if (f.equals(key)) {
	...
	private void serveError(final Response response, final OutputStream body,
			final Status status, final HTTPTrackerErrorMessage error)
			throws IOException {
	 ...
	 */
	private void serveError(final Response response, final OutputStream body,
			final Status status, final String error) throws IOException {
		try {
	 ...
	 *            The HTTP status code to return.
	 * @param reason
	 *            The failure reason reported by the tracker.
	 */
	private void serveError(final Response response, final OutputStream body,
			final Status status, final ErrorMessage.FailureReason reason)
			throws IOException {
