...
import old.ttorrent.client.Client;
...
public class SeederClient {
	 ...
	 *            the data file
	 */
	public SeederClient(final Properties configuration,
			final TrackedTorrent torrent, final Path torrentFile,
			final Path dataFile) {
		...
		try {
			if (totalNumberOfClient < 2 && seeder == null
				...
				seeder = new Client(InetAddress.getByName(trackerIp),
						sharedTorrent);
				seeder.share();
				LOGGER.info("Started seeding {}. Number of client {}",
						torrent.getName(), totalNumberOfClient);
			}
	...
	public synchronized void stopSeeding() {
		try {
			if (seeder != null) {
				seeder.stop(true);
				totalNumberOfClient -= 1;
				LOGGER.info("Stoped seeding {}. Number of client {}",
						torrent.getName(), totalNumberOfClient);
				seeder = null;
			}
			System.gc();
	...
	public synchronized void stopAndDelete() {
		try {
			LOGGER.info(
					"Trying to stop and delete {} and {}. Number of client {}",
					new Object[] { torrentFile, dataFile, totalNumberOfClient });
			stopSeeding();
			final String deleteString = configuration.getProperty(
			...
			if (Boolean.valueOf(deleteString)) {
				...
				;
				LOGGER.info(
						"Delete torrent, file and remove from tracker {}. Number of client {}",
						torrent.getName(), totalNumberOfClient);
			}
		} catch (final Exception e) {
			LOGGER.error("Cannot stop seeding:" + e.getMessage(), e);
	...
	public void enableDisableSeeding(final TrackedTorrent torrent) {
		...
		if (leechers > 0 && seeder == null) {
			LOGGER.info(torrentFile + ": {} leechers, {} seeders", leechers,
					seeders);
