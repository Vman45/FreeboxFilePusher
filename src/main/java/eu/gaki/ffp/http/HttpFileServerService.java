...
public class HttpFileServerService implements Container {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(HttpFileServerService.class);
	 ...
	 *            the http file server
	 */
	public HttpFileServerService(final Properties configuration,
			final HttpFileServer httpFileServer) {
	...
	public void handle(final Request request, final Response response) {
		...
		final String rssFileString = configuration.getProperty("rss.location",
				"rss.xml");
		final Path rssPath = FileSystems.getDefault().getPath(rssFileString);

		Path pathToServe = null;
		if (requestName != null
				&& requestName.equals(rssPath.getFileName().toString())) {
			pathToServe = rssPath;
		} else {
	 ...
	 */
	private void sendFile(final Request request, final Response response,
			final Path pathToServe, final boolean forceReadOnly) {
	 ...
	 *            the force read only (deletion forbiden)
	 */
	private void detectTransfertEnd(final Response response, final Path path,
			final boolean forceReadOnly) {

		try {
			if (response.getResponseTime() > 0) {
				// Get response observer
				final Field observerField = response.getClass()
						.getDeclaredField("observer");
				observerField.setAccessible(true);
				final Object responseObserver = observerField.get(response);
				// Get error flag value
				final Field errorField = responseObserver.getClass()
						.getDeclaredField("error");
				errorField.setAccessible(true);
				final AtomicBoolean error = (AtomicBoolean) errorField
	 ...
	 *            the force read only (deletion forbiden)
	 */
	private void sendAll(final Request request, final Response response,
			final Path file, final boolean forceReadOnly) {
	 ...
	 *            the force read only (deletion forbiden)
	 */
	private void sendRange(final Response response, final Path file,
			final String range, final boolean forceReadOnly) {
		...
		if (typeValue.length == 2 && "bytes".equals(typeValue[0].trim())) {
			...
			if (rangeIntervals.length == 1) {
				...
				try (WritableByteChannel output = response.getByteChannel();
					...
					final long firstBytePosition = Long
							.valueOf(rangeInterval[0].trim());

					// Compute the byte to end
					long lastBytePosition;
					if (rangeInterval.length == 2) {
						// A end is specified: use it
						lastBytePosition = Long
								.valueOf(rangeInterval[1].trim());
					} else {
					...
					response.setValue("Content-Disposition",
							"attachment; filename=\"" + file.getFileName()
									+ "\"");
					final long contentLength = lastBytePosition
							- firstBytePosition + 1;
					response.setContentLength(contentLength);
					response.setValue("Content-Range", "bytes "
