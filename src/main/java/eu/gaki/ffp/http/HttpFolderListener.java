...
public class HttpFolderListener implements FolderListener {
	...
	public HttpFolderListener(final Properties configuration)
		...
		final String serverPort = configuration.getProperty("http.server.port",
				"80");
		final String serverIp = configuration.getProperty("http.server.ip",
	...
	public boolean isAlreadyPushed(final Path path) {
		return httpFileServer.isFileServed(path)
				|| filesCompresor.isInProgress(path);
	}
	...
	public Set<RssFileItem> getRssItemList() {
		...
		final String httpUrlTemplate = configuration.getProperty("http.url",
		...
		filesToServe
				...
				.forEach(entry -> {
					...
					// Rss link name
						...
						rssFileItem.setUrl(httpUrlTemplate.replace(
								"${file.name}", nameUrl));
						// Rss file date
						try {
							final FileTime lastModifiedTime = Files
									.getLastModifiedTime(entryPath);
