 ...
 * @author mpetazzoni
 */
public class HTTPAnnounceRequestMessage extends HTTPTrackerMessage implements
	...
	private HTTPAnnounceRequestMessage(ByteBuffer data, byte[] infoHash,
			Peer peer, long uploaded, long downloaded, long left,
	...
	public URL buildAnnounceURL(URL trackerAnnounceURL)
		...
		url.append(base.contains("?") ? "&" : "?")
				.append("info_hash=")
				.append(URLEncoder.encode(new String(this.getInfoHash(),
						Torrent.BYTE_ENCODING), Torrent.BYTE_ENCODING))
				.append("&peer_id=")
				.append(URLEncoder.encode(new String(this.getPeerId(),
						Torrent.BYTE_ENCODING), Torrent.BYTE_ENCODING))
				.append("&port=").append(this.getPort()).append("&uploaded=")
				.append(this.getUploaded()).append("&downloaded=")
				.append(this.getDownloaded()).append("&left=")
				.append(this.getLeft()).append("&compact=")
				.append(this.getCompact() ? 1 : 0).append("&no_peer_id=")
	...
	public static HTTPAnnounceRequestMessage parse(ByteBuffer data)
		...
		try {
			...
			if (params.containsKey("event")) {
				event = RequestEvent.getByName(params.get("event").getString(
						Torrent.BYTE_ENCODING));
			}

			return new HTTPAnnounceRequestMessage(data, infoHash, new Peer(ip,
					port, ByteBuffer.wrap(peerId)), uploaded, downloaded, left,
	...
	public static HTTPAnnounceRequestMessage craft(byte[] infoHash,
			...
			boolean compact, boolean noPeerId, RequestEvent event, String ip,
			int numWant) throws IOException, MessageValidationException,
		...
		if (event != null) {
			params.put("event", new BEValue(event.getEventName(),
					Torrent.BYTE_ENCODING));
		}
		...
		return new HTTPAnnounceRequestMessage(BEncoder.bencode(params),
