...
public class FilesCompresor {
	...
	final LocalTime startDate = LocalTime.now();
	LOGGER.info("Start compress: "+pathToCompress);

	// Tar
	final Path tarXzFile = computeTarXzName(pathToCompress);
	try (final OutputStream tarXzOutputStream = Files.newOutputStream(tarXzFile, StandardOpenOption.CREATE,
		StandardOpenOption.TRUNCATE_EXISTING);
		final OutputStream tarXzBufferedOutputStream = new BufferedOutputStream(tarXzOutputStream);
		final OutputStream xzOutputStream = new XZCompressorOutputStream(tarXzBufferedOutputStream);
	    ...
	    // final String rootPath = pathToCompress.toString() + File.pathSeparator;
	    final Stream<Path> walk = Files.walk(pathToCompress, FileVisitOption.FOLLOW_LINKS);

	    final Stream<Path> walkFiltered = walk.filter(p -> {
		final String name = p.getFileName().toString();
		final String extension = FilenameUtils.getExtension(name);
		boolean include = false;

		if (Files.isDirectory(p)) {
		    // Exclude the root folder to compress (we tar sub path of the root folder)
		...
		try {
		    ...
		    // final String relativePath = t.toString().replace(rootPath, "");
	...
	final LocalTime endDate = LocalTime.now();
	final Duration between = Duration.between(startDate, endDate);

	LOGGER.info("Stop compress: " + pathToCompress + " took " + between.getSeconds() + " min");
	...
	final String excludeExtensionString = configuration.getProperty("fft.exclude.extension",
		".html,.exe,.txt,.readme,.nfo,.link");
