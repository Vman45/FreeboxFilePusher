...
public class HTTPTrackerClient extends TrackerClient {
	...
	protected static final Logger logger = LoggerFactory
			.getLogger(HTTPTrackerClient.class);
	...
	public void announce(AnnounceRequestMessage.RequestEvent event,
		...
		URL target = null;
		try {
			HTTPAnnounceRequestMessage request = this
					.buildAnnounceRequest(event);
		...
		try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
			...
			HTTPTrackerMessage message = HTTPTrackerMessage.parse(ByteBuffer
	 ...
	 *
	 * @param event
	 *            The announce event (can be <tt>NONE</tt> or <em>null</em>)
	 * @return Returns an instance of a {@link HTTPAnnounceRequestMessage} that
	 *         can be used to generate the fully qualified announce URL, with
	...
	private HTTPAnnounceRequestMessage buildAnnounceRequest(
		...
		return HTTPAnnounceRequestMessage.craft(this.torrent.getInfoHash(),
				this.peer.getPeerId().array(), this.peer.getPort(),
				this.torrent.getUploaded(), this.torrent.getDownloaded(),
				this.torrent.getLeft(), true, false, event, this.peer.getIp(),
