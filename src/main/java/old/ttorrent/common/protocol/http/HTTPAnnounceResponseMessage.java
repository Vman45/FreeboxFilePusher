 ...
 * @author mpetazzoni
 */
public class HTTPAnnounceResponseMessage extends HTTPTrackerMessage implements
	...
	private HTTPAnnounceResponseMessage(ByteBuffer data, int interval,
			int complete, int incomplete, List<Peer> peers) {
	...
	public static HTTPAnnounceResponseMessage parse(ByteBuffer data)
		...
		try {
			...
			return new HTTPAnnounceResponseMessage(data, params.get("interval")
					.getInt(), params.get("complete") != null ? params.get(
					"complete").getInt() : 0,
					params.get("incomplete") != null ? params.get("incomplete")
							.getInt() : 0, peers);
		} catch (InvalidBEncodingException ibee) {
	...
	/**
	 * Build a peer list as a list of {@link Peer}s from the announce response's
	 * peer list (in non-compact mode).
	 *
	 * @param peers
	 *            The list of {@link BEValue}s dictionaries describing the peers
	 *            from the announce response.
	 * @return A {@link List} of {@link Peer}s representing the peers'
	 *         addresses. Peer IDs are lost, but they are not crucial.
	 */
	private static List<Peer> toPeerList(List<BEValue> peers)
		...
		for (BEValue peer : peers) {
			...
			result.add(new Peer(peerInfo.get("ip").getString(
					Torrent.BYTE_ENCODING), peerInfo.get("port").getInt()));
		}
	...
	/**
	 * Build a peer list as a list of {@link Peer}s from the announce response's
	 * binary compact peer list.
	 *
	 * @param data
	 *            The bytes representing the compact peer list from the announce
	 *            response.
	 * @return A {@link List} of {@link Peer}s representing the peers'
	 *         addresses. Peer IDs are lost, but they are not crucial.
	 */
	...
	public static HTTPAnnounceResponseMessage craft(int interval,
		...
		return new HTTPAnnounceResponseMessage(BEncoder.bencode(response),
