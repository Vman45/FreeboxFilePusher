 ...
 * @author mpetazzoni
 * @see <a
 *      href="http://wiki.theory.org/BitTorrentSpecification#Metainfo_File_Structure">Torrent
...
public class Torrent {
	 ...
	 * @param seeder
	 *            Whether we'll be seeding for this torrent or not.
	 * @throws IOException
	 *             When the info dictionary can't be read or encoded and hashed
	...
	public Torrent(byte[] torrent, boolean seeder) throws IOException {
		...
		this.decoded = BDecoder.bdecode(new ByteArrayInputStream(this.encoded))
				.getMap();
		 ...
		 * Otherwise, the announce-list must be parsed and the trackers from
		 * each tier extracted.
		 * </p>
		 *
		...
		try {
			...
			if (this.decoded.containsKey("announce-list")) {
				List<BEValue> tiers = this.decoded.get("announce-list")
						.getList();
		...
		this.creationDate = this.decoded.containsKey("creation date") ? new Date(
				this.decoded.get("creation date").getLong() * 1000) : null;
		this.comment = this.decoded.containsKey("comment") ? this.decoded.get(
		...
		if (this.decoded_info.containsKey("files")) {
			for (BEValue file : this.decoded_info.get("files").getList()) {
				...
				this.files.add(new TorrentFile(new File(this.name, path
						.toString()), fileInfo.get("length").getLong()));
			}
		} else {
		...
		logger.info("  Announced at:"
				+ (this.trackers.size() == 0 ? " Seems to be trackerless" : ""));
		for (int i = 0; i < this.trackers.size(); i++) {
			List<URI> tier = this.trackers.get(i);
			for (int j = 0; j < tier.size(); j++) {
				logger.info("    {}{}", (j == 0 ? String.format("%2d. ", i + 1)
						: "    "), tier.get(j));
			}
		}
	 ...
	 *
	 * @return The list of relative filenames of all the files described in this
	 *         torrent.
	 */
	...
	/**
	 * Return an hexadecimal representation of the bytes contained in the given
	 * string, following the default, expected byte encoding.
	 *
	 * @param input
	 *            The input string.
	 */
	 ...
	 *
	 * <p>
	 * If the environment variable TTORRENT_HASHING_THREADS is set to an integer
	 * value greater than 0, its value will be used. Otherwise, it defaults to
	 * the number of processors detected by the Java Runtime.
	 * </p>
	 *
	 ...
	 *
	 * @param torrent
	 *            The abstract {@link File} object representing the
	 *            <tt>.torrent</tt> file to load.
	 * @throws IOException
	 *             When the torrent file cannot be read.
	 */
	 ...
	 *            <tt>.torrent</tt> file to load.
	 * @param seeder
	 *            Whether we are a seeder for this torrent or not (disables
	 ...
	 *
	 * <p>
	 * Hash the given file to create the {@link Torrent} object representing the
	 * Torrent metainfo about this file, needed for announcing and/or sharing
	 * said file.
	 * </p>
	 *
	 * @param source
	 *            The file to use in the torrent.
	 * @param announce
	 *            The announce URI that will be used for this torrent.
	 * @param createdBy
	 *            The creator's name, or any string identifying the torrent's
	 *            creator.
	 */
	public static Torrent create(File source, URI announce, String createdBy)
			throws InterruptedException, IOException {
		return Torrent.create(source, null, DEFAULT_PIECE_LENGTH, announce,
				null, createdBy);
	}
	 ...
	 *
	 * @param parent
	 *            The parent directory or location of the torrent files, also
	 *            used as the torrent's name.
	 * @param files
	...
	public static Torrent create(File parent, List<File> files, URI announce,
		...
		return Torrent.create(parent, files, DEFAULT_PIECE_LENGTH, announce,
				null, createdBy);
	}
	 ...
	 *            The file to use in the torrent.
	 * @param announceList
	 *            The announce URIs organized as tiers that will be used for
	 *            this torrent
	 * @param createdBy
	 *            The creator's name, or any string identifying the torrent's
	 *            creator.
	 */
	public static Torrent create(File source, int pieceLength,
			List<List<URI>> announceList, String createdBy)
			throws InterruptedException, IOException {
		return Torrent.create(source, null, pieceLength, null, announceList,
				createdBy);
	}
	...
	public static Torrent create(File source, List<File> files,
			int pieceLength, List<List<URI>> announceList, String createdBy)
			throws InterruptedException, IOException {
		return Torrent.create(source, files, pieceLength, null, announceList,
				createdBy);
	}
	 ...
	 *            creator.
	 */
	private static Torrent create(File parent, List<File> files,
			int pieceLength, URI announce, List<List<URI>> announceList,
			String createdBy) throws InterruptedException, IOException {
		if (files == null || files.isEmpty()) {
			logger.info("Creating single-file torrent for {}...",
					parent.getName());
		} else {
			logger.info("Creating {}-file torrent {}...", files.size(),
					parent.getName());
		}
		...
		if (files == null || files.isEmpty()) {
			...
			info.put("pieces",
					new BEValue(Torrent.hashFile(parent, pieceLength),
			...
			info.put("pieces",
					new BEValue(Torrent.hashFiles(files, pieceLength),
							Torrent.BYTE_ENCODING));
		}
	 ...
	 * length (see {@link #PIECE_LENGTH}) and returns the concatenation of these
	 * hashes, as a string.
	 * </p>
	 *
	 * <p>
	...
	private static String hashFiles(List<File> files, int pieceLenght)
		...
		for (File file : files) {
			logger.info(
					"Hashing data from {} with {} threads ({} pieces)...",
					new Object[] {
							file.getName(),
							threads,
							(int) (Math.ceil((double) file.length()
									/ pieceLenght)) });
			...
			try {
				while (channel.read(buffer) > 0) {
					if (buffer.remaining() == 0) {
						buffer.clear();
						results.add(executor.submit(new CallableChunkHasher(
								buffer)));
					}
		...
		logger.info(
				...
				new Object[] { files.size(), length, pieces, expectedPieces,
	 ...
	 *
	 * @param hashes
	 *            The {@link StringBuilder} to append hashes to.
	 * @param results
	 *            The list of {@link Future}s that will yield the piece hashes.
	 */
	private static int accumulateHashes(StringBuilder hashes,
			List<Future<String>> results) throws InterruptedException,
			IOException {
		try {
