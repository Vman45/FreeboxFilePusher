...
public abstract class TrackerClient {
	 ...
	 *
	 * <p>
	 * This method is called by {@link Announce#stop()} to make sure all
	 * connections are correctly closed when the announce thread is asked to
	 * stop.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * Analyzes the response from the tracker and acts on it. If the response is
	 * an error, it is logged. Otherwise, the announce response is used to fire
	 * the corresponding announce and peer events to all announce listeners.
	 * </p>
	 *
	 * @param message
	 *            The incoming {@link TrackerMessage}.
	 * @param inhibitEvents
	...
	protected void handleTrackerAnnounceResponse(TrackerMessage message,
		...
		this.fireAnnounceResponseEvent(response.getComplete(),
				response.getIncomplete(), response.getInterval());
