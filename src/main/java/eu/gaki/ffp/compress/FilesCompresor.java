...
public class FilesCompresor {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(HttpFolderListener.class);
	...
	private void doCompress(final Path pathToCompress) {
		...
		final String compressionMethodString = configuration.getProperty(
				"ffp.compress.method", "8");
		final int compressionMethod = Integer.valueOf(compressionMethodString);
		final String compressionLevelString = configuration.getProperty(
				"ffp.compress.level", "0");
		final int compressionLevel = Integer.valueOf(compressionLevelString);
		...
		try (ZipArchiveOutputStream zipOutputStream = new ZipArchiveOutputStream(
			...
			walkFiltered.forEach(t -> {
				try {
					...
					final ZipArchiveEntry entry = new ZipArchiveEntry(t
							.toFile(), relativePath.normalize().toString());
	...
	private Collection<String> getExcludeExtensions() {
		...
		final String excludeExtensionString = configuration.getProperty(
				"fft.exclude.extension", ".html,.exe,.txt,.readme,.nfo,.link");
	...
	public Path computeCompressFileName(final Path pathToCompress) {
		...
		final Path parent = pathToCompress.getParent();
		if (pathToCompress != null && parent != null) {
			compressFileName = parent.resolve("" + pathToCompress.getFileName()
					+ ZIP_EXTENSION);
		}
